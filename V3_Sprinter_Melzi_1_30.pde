...
const char* pszFirmware[]        = { "Sprinter", "https://github.com/smalcolmbrown/V3-Sprinter-Melzi_1_20/", _VERSION_TEXT, "Vector 3", "1" };
...
//Send Temperature in °C to Host and LCD
int hotendtC = 0 ;                      // hook to Extruder temperature in C
int bedtempC = 0 ;                      // hook to Heated Bed temperature in C
int iHotEndTargetTemperatureC = 0 ;     // Extruder target temperature in C
int iBedTargetTemperatureC = 0 ;        // Heated Bed target temperature in C
...
#if FAN_PIN > -1
  #define FAN_COUNT 
  unsigned char fanSpeeds[FAN_COUNT] = { 0,0 };    // FAN_COUNT = 2 in the V3 initialise fans to 0
#endif
...
void EmergencyStop() {
  ...
  gcode_M107();                                                   // stop fan
  Serial.println("Emergency Stop");                               // tell the world
...
//   {
  ...
  #endif  //  #if (MINIMUM_FAN_START_SPEED > 0)
...
FORCE_INLINE void homing_routine(char axis)
{
  ...
  if ((min_pin > -1 && home_dir==-1) || (max_pin > -1 && home_dir==1))
  {
    ...
    if(axis==Z_AXIS)
    {
      current_position[axis] = (home_dir == -1) ? (float)byteToint(EEPROM.read(Z_ADJUST_BYTE))/100 : Z_MAX_LENGTH_M240;
    }
    else
    {
...
FORCE_INLINE void process_commands()
{
...
#endif  //  #ifdef SEND_WRONG_CMD_INFO
  ...
  else if(code_seen('M'))
  {
    ...
    switch( (int)code_value() ) 
    {
      ...
      case 30: // M30 filename - Delete file or M30 - fast SD transfer
        gcode_M30();
        break;  
   ...
   #endif  //  #ifndef SD_FAST_XFER_AKTIV
...
//        break;
      ...
      case 203: // M203 Temperature monitor or Set Z height adjustment if V3
        gcode_M203();
        break;
      ...
      case 237: // M237	HSW Enable
        HSW_Enable = 0x01;
        showString(PSTR("M237 OK\r\n"));
        break;

      case 238:
        HSW_Enable = 0x00;
        showString(PSTR("M238 OK\r\n"));
        break;
...
////////////////////////////////
// G2 - Controlled Clockwise Arc Move C
//
////////////////////////////////
...
//  Save this value into the EEPROM
  */
  ...
  showString(PSTR("Bed X: "));                            // tell the controling software 
  Serial.print(fX_Probe + 0.0001);                       // the result
  showString(PSTR(" Y: "));
  Serial.print(fY_Probe + 0.0001);
  showString(PSTR(" Z: "));
  Serial.println(fZ_Height + 0.0001);
  }
  previous_millis_cmd = millis();
...
inline void gcode_M4()
{
  ...
  Serial.print(status);
  showString(PSTR(", "));
  Serial.println(pszStatusString[status]);
  if(status == STATUS_ERROR)
  {
    showString(PSTR("EC:"));
    Serial.print(error_code);
    showString(PSTR(", "));
    Serial.println(pszErrorCodeString[error_code]);
  }
}
...
//
// depending on the SD_FAST_XFER_AKTIV switch
////////////////////////////////
...
inline void gcode_M30()
{
  ...
  #else // else of #ifndef SD_FAST_XFER_AKTIV
...
inline void gcode_M31()
{
  ...
  #endif  // #ifdef SD_FAST_XFER_AKTIV
...
inline void gcode_M42() {
  ...
  if(code_seen ( 'S' ))
  {
    ...
    else if( code_seen('P') && iPinStatus >= 0 && iPinStatus <= 255)
    {
      ...
      if (iPinNumber <27 || iPinNumber >30)
      {
        // only pins A1 A2 A3 A4 allowed on the V3
        iPinNumber = -1;
      }
    }
    else
    {
      // neither P or A specified so LED_PIN is switched (27)
      iPinNumber = LED_PIN;
    }

#else  // else of #ifdef V3
     
    if (code_seen('P') && iPinStatus >= 0 && iPinStatus <= 255)
    {
      iPinNumber = code_value();
      for(int i = 0; i < sizeof(sensitive_pins) / sizeof(int); i++)
      {
        if (sensitive_pins[i] == iPinNumber)
        {
          iPinNumber = -1;
          break;
        }
    ...
    {
      // P NOT specified so LED_PIN is switched (27)
      iPinNumber = LED_PIN;
    }
...
inline void gcode_M93()
{
  showString(PSTR("ok "));
  showString(PSTR("X:"));
  Serial.print(axis_steps_per_unit[X_AXIS]);
  showString(PSTR("Y:"));
  Serial.print(axis_steps_per_unit[Y_AXIS]);
  showString(PSTR("Z:"));
  Serial.print(axis_steps_per_unit[Z_AXIS]);
  showString(PSTR("E:"));
  Serial.println(axis_steps_per_unit[E_AXIS]);
}
...
inline void gcode_M104() 
{
  ...
  if (code_seen('S')) 
  {
    iHotEndTargetTemperatureC = code_value();
    target_raw = temp2analogh(iHotEndTargetTemperatureC);
  }
...
#endif  //  #if (TEMP_0_PIN > -1) || defined (HEATER_USES_MAX6675)|| defined HEATER_USES_AD595
...
#else  // of #if (TEMP_0_PIN > -1) || defined (HEATER_USES_MAX6675) || defined HEATER_USES_AD595
...
#endif  // #if defined(FAN_SOFT_PWM) && (FAN_PIN > -1)
...
#else  // else of #ifdef TEMP_RESIDENCY_TIME
              
  while ( target_direction ? (current_raw < target_raw) : (current_raw > target_raw) ) 
  {

#endif  //  #ifdef TEMP_RESIDENCY_TIME
    ...
    if( (millis() - codenum) > 1000 ) //Print Temp Reading every 1 second while heating up/cooling down
    {
      ...
      StatusScreen();                                            // update the extruder temperature
...
inline void gcode_M140() 
{
  ...
  if (code_seen('S'))
  {
    iBedTargetTemperatureC = code_value();
    target_bed_raw = temp2analogBed(iBedTargetTemperatureC);
    
#ifdef LCD_SUPPORTED

    StatusScreen();                                            // update the bed temperature
    
#endif  // #ifdef LCD_SUPPORTED
...
#else   // Not used for Sprinter/grbl gen6
...
#endif // Not used for Sprinter/grbl gen6
...
inline void gcode_M203() 
{
  ...
  if(code_seen('Z')) EEPROM.write(Z_ADJUST_BYTE,code_value()*100);

#else      // Non V3 specific code

  if(code_seen('S')) manage_monitor = code_value();
  if(manage_monitor==100) manage_monitor=1; // Set 100 to heated bed
  
#endif     // V3 specific code
}
...
////////////////////////////////
// M205 advanced settings:  minimum travel speed
// S=while printing T=travel only
// B=minimum segment time X= maximum xy jerk
// Z=maximum Z jerk
// E= max E jerk
//
////////////////////////////////
...
inline void gcode_M261() {
  ...
  if (i2c.Address() && byData && byData <= EI2C_BUS_BUFFER_SIZE) {
    ...
    showString(PSTR("Bad i2c request"));
    showString(PSTR("\r\n"));
  }
}
...
inline void gcode_M355() {
  ...
  if( iPinStatus > -1)
  {
    showString(PSTR("Case lights O"));
    Serial.println((iPinStatus) ? "n" : "ff");
  }
}
...
void ErrorBleepCodes()
{
  ...
  showString(PSTR("EC:"));                                        // Now send error code and error string to the terminal
  Serial.print(error_code);
  showString(PSTR(", "));
  Serial.print(pszErrorCodeString[error_code]);
  showString(PSTR("\r\n"));
