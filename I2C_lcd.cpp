...
extern int hotendtC ;                          // hook to Extruder temperature in C
extern int bedtempC ;                          // hook to Heated Bed temperature in C
extern int iHotEndTargetTemperatureC ;         // hook to Extruder target temperature in C
extern int iBedTargetTemperatureC ;            // hook to Heated Bed target temperature in C
...
void PrinterState( ){
  switch( iPrinterState){
    case STATE_IDLE:
      ...
      if ( iHotEndTargetTemperatureC || iBedTargetTemperatureC) {
    ...
    case STATE_HEATING:
      ...
      if( hotendtC >= (iHotEndTargetTemperatureC -5) && (iHotEndTargetTemperatureC != 0 )){
...
void DisplayBedAndExtruderTemparature() {
  // extruder 
  lcd.setCursor( 0, 0 ); 
  sprintf( szTemp, "%c%3d/%d%c     ", LCD_THERMOMETER, hotendtC, iHotEndTargetTemperatureC, LCD_DEGREE_CHAR );
  *(szTemp+10) = 0;               // truncate to 10 letters
  lcd.print( szTemp );
  // heated bed
  lcd.setCursor( 10, 0 ); 
  sprintf( szTemp, "%c%3d/%d%c     ", LCD_BEDTEMP_CHAR, bedtempC, iBedTargetTemperatureC, LCD_DEGREE_CHAR );
  *(szTemp+10) = 0;               // truncate to 10 letters
  lcd.print( szTemp );
}
